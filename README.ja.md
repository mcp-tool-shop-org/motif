...
| [`@motif-studio/audio-engine`](packages/audio-engine) | サンプル再生、ボイス管理、AudioContextのスケジュール |
| [`@motif-studio/test-kit`](packages/test-kit) | テスト用ファイルとユーティリティ |
...
| [`@motif-studio/instrument-rack`](packages/instrument-rack) | マルチオシレーター・シンセ、ドラムボイス、サンプルボイス、LFOモジュレーション、16種類のプリセット |
| [`@motif-studio/music-theory`](packages/music-theory) | スケール、コード、モチーフ、インテンシティ変換 |
| [`@motif-studio/playback-engine`](packages/playback-engine) | リアルタイム再生、ミキシング、8種類のイフェクト、MIDI入出力、WAVエクスポート（24/32ビット） |
| [`@motif-studio/sample-lab`](packages/sample-lab) | トリミング、スライス、キット、楽器ヘルパー |
| [`@motif-studio/score-map`](packages/score-map) | モチーフ、プロファイル、キューファミリー、派生 |
...
| [`@motif-studio/runtime-pack`](packages/runtime-pack) | 決定論的なシリアル化によるランタイムエクスポート/インポート |
| [`@motif-studio/review`](packages/review) | サマリーと監査ヘルパー |
...
すべてのパッケージは、npm上で`@motif-studio`スコープで公開されています。
