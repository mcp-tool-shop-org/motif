...
| 包 | 描述 |
|---------|-------------|
| [`@motif-studio/schema`](packages/schema) | 标准类型、Zod模式、解析/验证 |
| [`@motif-studio/asset-index`](packages/asset-index) | 打包完整性索引和审计 |
| [`@motif-studio/audio-engine`](packages/audio-engine) | 采样播放、音色管理、音频上下文调度 |
| [`@motif-studio/test-kit`](packages/test-kit) | 测试工具和辅助程序 |

### 创作和播放

| 包 | 描述 |
|---------|-------------|
| [`@motif-studio/clip-engine`](packages/clip-engine) | 片段序列、变换、提示调度 |
| [`@motif-studio/instrument-rack`](packages/instrument-rack) | 多振荡器合成器、鼓音色、采样音色、LFO调制、16种预设 |
| [`@motif-studio/music-theory`](packages/music-theory) | 音阶、和弦、动机、强度变换 |
| [`@motif-studio/playback-engine`](packages/playback-engine) | 实时播放、混音、8种效果类型、MIDI输入/输出、WAV导出（24/32位） |
| [`@motif-studio/sample-lab`](packages/sample-lab) | 裁剪、分割、音色包、乐器辅助工具 |
| [`@motif-studio/score-map`](packages/score-map) | 动机、配置文件、提示族、衍生 |
| [`@motif-studio/automation`](packages/automation) | 音轨、宏、包络线、捕捉 |
...
| 包 | 描述 |
|---------|-------------|
| [`@motif-studio/scene-mapper`](packages/scene-mapper) | 触发器映射和确定性绑定评估 |
| [`@motif-studio/runtime-pack`](packages/runtime-pack) | 具有确定性序列化的运行时导出/导入 |
| [`@motif-studio/review`](packages/review) | 摘要和审计辅助工具 |
| [`@motif-studio/ui`](packages/ui) | 共享用户界面组件 |
