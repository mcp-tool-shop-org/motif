...
Motif 是一款以创作为先导，并具有自适应功能的专业工作站。它将结构化的音乐创作功能（包括音符、提示、场景、音轨和自动化）与能够根据游戏状态实时响应的自适应逻辑相结合。结果是，游戏音乐听起来更自然，而不是由程序生成的。

## 它不是什么

它不是一个数字音频工作站 (DAW)。它不是一个简单的音序器。它不是一个人工智能音乐生成器。它也不是一个带有音效的世界构建数据库。Motif 是一个用于自适应游戏配乐创作的专业创意工具。
...
| 包 | 描述 |
|---------|-------------|
| [`@motif/schema`](packages/schema) | 标准类型、Zod 模式、解析/验证 |
| [`@motif/asset-index`](packages/asset-index) | 打包完整性索引和审计 |
| [`@motif/audio-engine`](packages/audio-engine) | 采样播放、音色管理、AudioContext 调度 |
...
| 包 | 描述 |
|---------|-------------|
| [`@motif/clip-engine`](packages/clip-engine) | 音符序列、变换、提示调度 |
| [`@motif/instrument-rack`](packages/instrument-rack) | 多振荡器合成器、鼓音色、采样音色、LFO 调制、16 种预设 |
| [`@motif/music-theory`](packages/music-theory) | 音阶、和弦、动机、强度变换 |
| [`@motif/playback-engine`](packages/playback-engine) | 实时播放、混音、8 种效果类型、MIDI 输入/输出、WAV 导出（24/32 位） |
| [`@motif/sample-lab`](packages/sample-lab) | 裁剪、切片、鼓组、乐器辅助功能 |
...
| [`@motif/runtime-pack`](packages/runtime-pack) | 带有确定性序列化的运行时导出/导入 |
| [`@motif/review`](packages/review) | 摘要和审计辅助程序 |
...
[手册](https://mcp-tool-shop-org.github.io/motif/handbook/product/)是全面的操作手册，涵盖产品定义、架构、工作室导航、创意工作流程和策略。主要入口点：

- [产品：Motif是什么](https://mcp-tool-shop-org.github.io/motif/handbook/product/)
...
- [工作流程：使用自定义示例](https://mcp-tool-shop-org.github.io/motif/handbook/workflows/custom-samples/)
